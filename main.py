...
async def main(token: str, guild_id):
    headers = {**Tools.auth_headers(token), "Content-Type": "application/json"}
    System.Clear()
    Funcs.print_logo()
    global info
        ...
        if args[1].endswith(".json") and os.path.exists(args[1]):
            with open(args[1], "r", encoding="utf8") as fp:
                try:
                    ...
                    Logger.Error.error("Invalid session JSON file format.")
                    j = {}

            _in = lambda t: j.get(t) if isinstance(j, dict) else None
